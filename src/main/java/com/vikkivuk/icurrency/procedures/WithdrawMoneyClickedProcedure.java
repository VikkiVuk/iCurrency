...
public class WithdrawMoneyClickedProcedure {
	public static void execute(Entity entity, HashMap guistate) {
		...
		if (Math.round(new Object() {
			...
			if (entity.getData(IcurrencyModVariables.PLAYER_VARIABLES).money >= Math.round(new Object() {
				double convert(String s) {
					try {
						return Double.parseDouble(s.trim());
					} catch (Exception e) {
				...
				total_amount = total_amount - 10 * (Math.floor(total_amount / 100) >= 64 ? 64 : Math.floor(total_amount / 100));
				{
					...
					_vars.money = entity.getData(IcurrencyModVariables.PLAYER_VARIABLES).money - Math.round(new Object() {
						double convert(String s) {
							try {
								return Double.parseDouble(s.trim());
							} catch (Exception e) {
