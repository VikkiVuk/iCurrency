...
public class COCardFourNumberProcedure {
	public static String execute(Entity entity) {
		...
		if (entity.getData(McurrencyModVariables.PLAYER_VARIABLES).cards.containsKey("four")) {
			...
			}).get(entity.getData(McurrencyModVariables.PLAYER_VARIABLES).cards, "four")).x()) + " " + Math.round(((new Object() {
				public Vec3 get(HashMap<String, Vec3> hashMap, String key) {
					Vec3 vec3 = hashMap.get(key);
					return vec3 == null ? Vec3.ZERO : vec3;
				}
			}).get(entity.getData(McurrencyModVariables.PLAYER_VARIABLES).cards, "four")).y());
		}
		return "UNKNOWN";
	}
}
