...
import com.vikkivuk.mcurrency.world.inventory.SetCardPinMenu;
...
public class DebitCardRightClickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (!itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getBoolean("activated")) {
			if (entity.getData(McurrencyModVariables.PLAYER_VARIABLES).cards.size() > 4) {
				...
				if (!entity.getData(McurrencyModVariables.PLAYER_VARIABLES).cards.containsKey("one")) {
					entity.getData(McurrencyModVariables.PLAYER_VARIABLES).cards.put("one", (new Vec3((itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("h1n")),
				...
				} else if (!entity.getData(McurrencyModVariables.PLAYER_VARIABLES).cards.containsKey("two")) {
					entity.getData(McurrencyModVariables.PLAYER_VARIABLES).cards.put("two", (new Vec3((itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("h1n")),
							(itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("h2n")), (itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("cvc")))));
				} else if (!entity.getData(McurrencyModVariables.PLAYER_VARIABLES).cards.containsKey("three")) {
					entity.getData(McurrencyModVariables.PLAYER_VARIABLES).cards.put("three", (new Vec3((itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("h1n")),
							(itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("h2n")), (itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("cvc")))));
				} else if (!entity.getData(McurrencyModVariables.PLAYER_VARIABLES).cards.containsKey("four")) {
					entity.getData(McurrencyModVariables.PLAYER_VARIABLES).cards.put("four", (new Vec3((itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("h1n")),
							(itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("h2n")), (itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("cvc")))));
				}
				if (entity instanceof Player _player) {
					McurrencyModVariables.PlayerVariables _vars = _player.getData(McurrencyModVariables.PLAYER_VARIABLES);
			...
			if (entity instanceof Player _player && !_player.level().isClientSide())
				_player.displayClientMessage(Component.literal((itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getString("pin"))), true);
		}
	}
}
