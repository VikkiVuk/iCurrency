...
import com.vikkivuk.mcurrency.procedures.CashRegisterPayCardValueProcedure;
import com.vikkivuk.mcurrency.network.CashRegisterPayCardButtonMessage;
...
public class CashRegisterPayCardScreen extends AbstractContainerScreen<CashRegisterPayCardMenu> {
	...
	public static HashMap<String, String> getEditBoxAndCheckBoxValues() {
		...
		if (Minecraft.getInstance().screen instanceof CashRegisterPayCardScreen sc) {

		}
		return textstate;
	}

	private static final ResourceLocation texture = ResourceLocation.parse("mcurrency:textures/screens/cash_register_pay_card.png");
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (mouseX > leftPos + 147 && mouseX < leftPos + 171 && mouseY > topPos + 4 && mouseY < topPos + 28)
			guiGraphics.renderTooltip(font, Component.translatable("gui.mcurrency.cash_register_pay_card.tooltip_void_the_transaction"), mouseX, mouseY);
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		guiGraphics.drawString(this.font, Component.translatable("gui.mcurrency.cash_register_pay_card.label_card_payment"), 7, 6, -12829636, false);
	}

	@Override
	public void init() {
		super.init();
		button_swipe_card = Button.builder(Component.translatable("gui.mcurrency.cash_register_pay_card.button_swipe_card"), e -> {
			if (true) {
				PacketDistributor.sendToServer(new CashRegisterPayCardButtonMessage(0, x, y, z, getEditBoxAndCheckBoxValues()));
				CashRegisterPayCardButtonMessage.handleButtonAction(entity, 0, x, y, z, getEditBoxAndCheckBoxValues());
			}
		...
		imagebutton_trash = new ImageButton(this.leftPos + 151, this.topPos + 7, 16, 16, new WidgetSprites(ResourceLocation.parse("mcurrency:textures/screens/trash.png"), ResourceLocation.parse("mcurrency:textures/screens/trash_hover.png")), e -> {
			if (true) {
				PacketDistributor.sendToServer(new CashRegisterPayCardButtonMessage(1, x, y, z, getEditBoxAndCheckBoxValues()));
				CashRegisterPayCardButtonMessage.handleButtonAction(entity, 1, x, y, z, getEditBoxAndCheckBoxValues());
			}
		}) {
			@Override
