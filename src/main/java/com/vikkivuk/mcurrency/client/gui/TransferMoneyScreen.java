...
import com.vikkivuk.mcurrency.network.TransferMoneyButtonMessage;
...
public class TransferMoneyScreen extends AbstractContainerScreen<TransferMoneyMenu> {
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		guiGraphics.drawString(this.font, Component.translatable("gui.mcurrency.transfer_money.label_amount"), 56, 10, -12829636, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.mcurrency.transfer_money.label_receiver"), 50, 43, -12829636, false);
	}

	@Override
	public void init() {
		super.init();
		receiver = new EditBox(this.font, this.leftPos + 12, this.topPos + 54, 118, 18, Component.translatable("gui.mcurrency.transfer_money.receiver"));
		receiver.setMaxLength(32767);
		guistate.put("text:receiver", receiver);
		this.addWidget(this.receiver);
		amount = new EditBox(this.font, this.leftPos + 12, this.topPos + 21, 118, 18, Component.translatable("gui.mcurrency.transfer_money.amount"));
		amount.setMaxLength(32767);
		guistate.put("text:amount", amount);
		this.addWidget(this.amount);
		button_empty = Button.builder(Component.translatable("gui.mcurrency.transfer_money.button_empty"), e -> {
			if (true) {
		...
		}).bounds(this.leftPos + 11, this.topPos + 76, 119, 20).build();
